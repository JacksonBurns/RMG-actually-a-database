...
if __name__ == "__main__":
    ...
    for i in range(len(reaction_list)):
        ...
        try:
            entry.long_desc = (
                "Originally from reaction library: "
                + reaction.library
                + "\n"
    ...
    kinetics_library.save(
        os.path.join(database_directory, "kinetics", "libraries", name, "reactions.py")
    )
    kinetics_library.save_dictionary(
        os.path.join(
            database_directory, "kinetics", "libraries", name, "dictionary.txt"
        )
    )
