...
def generate_evans_polanyi_plot(depository, database):
    ...
    for entry in entries:
        if isinstance(entry.data, Arrhenius):
            ...
            Ea.append(reaction.kinetics.Ea.value / 1000.0)
            dHrxn.append(reaction.get_enthalpy_of_reaction(298) / 1000.0)
    ...
    pylab.plot(
        dHrxnEP, EaEP + 1.96 * stdevEP, color="red", linestyle="dashed", linewidth=1
    )
    pylab.plot(
        dHrxnEP, EaEP - 1.96 * stdevEP, color="red", linestyle="dashed", linewidth=1
    )
    ...
    fig.subplots_adjust(
        left=0.14, bottom=0.1, top=0.95, right=0.95, wspace=0.20, hspace=0.20
    )
    ...
    def on_pick(event):
        ...
        for ind in event.ind:
            ...
            for reactant in reaction.reactants:
                print(
                    "%24s H298 = %g kcal/mol"
                    % (reactant, reactant.thermo.get_enthalpy(300) / 4184.0)
                )
            for product in reaction.products:
                print(
                    "%24s H298 = %g kcal/mol"
                    % (product, product.thermo.get_enthalpy(300) / 4184.0)
                )
